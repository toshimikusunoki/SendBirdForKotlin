...
class CreateGroupChannelActivity : AppCompatActivity(), SelectUserFragment.UsersSelectedListener, SelectDistinctFragment.DistinctSelectedListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mNextButton = findViewById<View>(R.id.button_create_group_channel_next) as Button
        ...
        mCreateButton = findViewById<View>(R.id.button_create_group_channel_create) as Button
        mCreateButton!!.setOnClickListener {
            if (mCurrentState == STATE_SELECT_USERS) {
                //                if (mCurrentState == STATE_SELECT_DISTINCT) {
                mIsDistinct = PreferenceUtils.groupChannelDistinct
                createGroupChannel(mSelectedIds, mIsDistinct)
            }
        }
        mCreateButton!!.isEnabled = false

        mToolbar = findViewById<View>(R.id.toolbar_create_group_channel) as Toolbar
    ...
    private fun createGroupChannel(userIds: List<String>?, distinct: Boolean) {
        GroupChannel.createChannelWithUserIds(userIds, distinct, GroupChannel.GroupChannelCreateHandler { groupChannel, e ->
            if (e != null) {
                // Error!
    ...
    companion object {
        ...
        internal val STATE_SELECT_USERS = 0
        internal val STATE_SELECT_DISTINCT = 1
    }
